...
```
Stage 1 (Current):   One server, one couple, centralized
                     ┌─────────┐
                     │  DO     │
                     │  suppr  │
                     └─────────┘

Stage 2 (Second):    Two servers, two couples, independent
                     ┌─────────┐     ┌─────────┐
                     │  Our    │     │  Their  │
                     │  suppr  │     │  suppr  │
                     └─────────┘     └─────────┘

Stage 3 (Linked):    Two servers, federated
                     ┌─────────┐────▶┌─────────┐
                     │  Our    │◀────│  Their  │
                     │  suppr  │     │  suppr  │
                     └─────────┘     └─────────┘

Stage 4 (Groups):    Mesh of servers
                     ┌─────────┐────▶┌─────────┐
                     │  A      │◀────│  B      │
                     └────┬────┘     └────┬────┘
                          │               │
...
Stage 5 (Local):     Desktop IS the server, DO becomes optional relay
```
